...
This C++ code snippet is part of the `moteus` project and implements an asynchronous UART (Universal Asynchronous Receiver-Transmitter) for STM32G4 microcontrollers. The class `Stm32G4AsyncUart` provides asynchronous communication capabilities over UART using DMA (Direct Memory Access) for efficient data transfer.

### Overview of `Stm32G4AsyncUart`:

- The `Stm32G4AsyncUart` class allows for asynchronous reading and writing of data over UART, which is beneficial for non-blocking communication in embedded systems.
- It uses DMA for data transfer, which offloads the CPU from the task of moving data byte by byte and enables efficient communication, especially at high data rates.
- The class is designed to work with the STM32G4 series of microcontrollers.

### Key Components:

1. **Constructor (`Stm32G4AsyncUart`)**: Initializes the UART with specified options like baud rate and pin configuration. It sets up DMA channels for both transmission (TX) and reception (RX).

2. **`AsyncReadSome` Method**: Asynchronously reads data from the UART into a provided buffer. It uses a callback to signal the completion of the read operation.

3. **`AsyncWriteSome` Method**: Asynchronously writes data to the UART from a given buffer. A callback is used to notify when the write operation is complete.

4. **`Poll` Method**: Needs to be called regularly to process asynchronous events, such as checking if a read or write operation has completed.

5. **DMA Configuration and Control**: The implementation involves setting up DMA channels, handling DMA interrupts, and managing data transfer between memory and the UART peripheral.

### Usage:

This class is used in embedded systems where non-blocking UART communication is required. It's particularly useful in scenarios where data needs to be sent or received continuously or in large amounts, such as in communication with sensors, other microcontrollers, or computers.

### Advantages:

- **Efficiency**: By using DMA, the CPU is freed from the overhead of handling each byte of data, which is especially beneficial at higher baud rates.
- **Non-blocking**: Asynchronous methods allow the main program to continue executing while the UART operations are carried out in the background.
- **Buffer Management**: Implements a buffer system to store incoming data between reads, reducing the risk of data loss in high-speed communications.

### Considerations:

- This implementation is specific to STM32G4 microcontrollers and uses direct register manipulation and STM32 HAL (Hardware Abstraction Layer) functions.
- Proper handling of DMA and UART interrupts is crucial for reliable operation.
- The user of this class needs to ensure `Poll` is called regularly to handle asynchronous events.
