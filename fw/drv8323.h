...
urpose: Interface for the DRV8323 motor driver.
Key Components:
Drv8323 class: Manages a DRV8323 motor driver.
Options struct: Pin configuration for SPI communication and motor driver control.
Status struct: Represents various fault and operational statuses of the motor driver.
Config struct: Configuration options for the motor driver, including PWM mode, gate drive strength, and overcurrent protection settings.
Functionality: Includes methods to enable/disable the driver, handle power and fault status, and perform periodic tasks (PollMillisecond).
Special Note: Configuration parameters are set dynamically based on the hardware revision detected.
#pragma once
