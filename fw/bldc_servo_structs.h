...
This header file defines various structures and enumerations used in controlling BLDC (Brushless DC) servo motors within the moteus namespace. Key components include:

Vec3: Represents a 3D vector with float values, typically used for 3-phase motor-related calculations.
BldcServoMode: An enumeration that defines various operational modes of the BLDC servo, such as kStopped, kFault, kPosition, etc.
BldcServoStatus: A structure containing detailed status of the BLDC servo, including current mode, fault status, raw ADC values, computed currents and voltages, motor and FET temperatures, PID controller states, and more. It's a comprehensive snapshot of the servo's current state.
BldcServoCommandData: Represents command data for controlling the BLDC servo, with fields corresponding to different control modes. It includes desired PWM values, phase voltages, position, velocity, torque, etc.
BldcServoMotor: Contains configuration parameters specific to the motor, such as pole count, phase inversion, resistance, inductance, and cogging compensation values.
BldcServoConfig: A structure that stores overall configuration parameters for the BLDC servo control, including PWM rate, current gain, PID configurations, limits for voltage, power, temperature, etc.
BldcServoPositionConfig: Stores positional limits for the servo.
